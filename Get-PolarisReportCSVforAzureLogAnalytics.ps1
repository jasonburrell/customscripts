...
​
    ...
    Defines where to save the CSV, for example: 'c:\rubriktemp\temp.csv'
​
    .INPUTS
​
    None. 
...
​
    ...
    if ($result) {
        ...
        #If Outpath defined write CSV while fixing the date/time format
        if ($OutputPath) {
            Invoke-WebRequest -Uri $link -OutFile $OutputPath
            #Fix Date/Time format
            $eventreport = Import-Csv -Path $OutputPath
                $output = ForEach ($event in $eventreport){
                $event.'Start time' = ([datetime]::ParseExact($event.'Start time', "MMM dd, yyyy h:mmtt UTC", $null)).ToString("yyyyMMdd HH:mm:ss")
                $event.'End time' = ([datetime]::ParseExact($event.'End time', "MMM dd, yyyy h:mmtt UTC", $null)).ToString("yyyyMMdd HH:mm:ss")
                $event
                }
            #$logname =  + (get-date).ToString("dd-MMM-yyyy") + ".csv"
...
$reportlink = Get-PolarisReportCSV -Token $token -PolarisURL $url -ReportID '584' -OutputPath ('c:\rubriklogs\rubrikeventlog' + (get-date).ToString("dd-MMM-yyyy") + ".csv")
